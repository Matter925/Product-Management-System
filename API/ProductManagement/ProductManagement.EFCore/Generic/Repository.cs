...
namespace ProductManagement.EFCore.Generic
{
    ...
    public sealed class Repository<TEntity> : IRepository<TEntity> where TEntity : class
    {
        ...
        {
            var result = await _context.Set<TEntity>().AnyAsync(anyExpression, cancellationToken).ConfigureAwait(false);
            return result;
        }
        ...
        {
            var count = await _context.Set<TEntity>().Where(whereExpression).CountAsync(cancellationToken).ConfigureAwait(false);
            return count;
        }
        ...
        {
            var count = await _context.Set<TEntity>().CountAsync(cancellationToken).ConfigureAwait(false);
            return count;
        }
        ...
        public async Task<int> CountAsync<TEntity, TFilter>(TFilter filter, CancellationToken cancellationToken = default)
            ...
            where TFilter : FilterBase
        {
            var count = await _context.Set<TEntity>().ApplyFilter(filter).CountAsync(cancellationToken).ConfigureAwait(false);
            return count;
        }
        ...
        private Expression<Func<TEntity, bool>> GenerateExpression<TEntity>(object id)
        {
            ...
            var pk = type.FindPrimaryKey().Properties.Select(s => s.Name).FirstOrDefault();
            var pkType = type.FindPrimaryKey().Properties.Select(p => p.ClrType).FirstOrDefault();
            ...
            var expression = Expression.Lambda<Func<TEntity, bool>>(body, new[] { pe });
        ...
        public Task HardDeleteRangeAsync<TEntity>(IEnumerable<TEntity> entities) where TEntity : class
        {
        ...
        public IEnumerable<string> GetTableNamesInSchema(string schemaName)
        {
            var tableNames = _context.Model.GetEntityTypes()
                .Where(t => t.GetSchema() == schemaName && !string.IsNullOrEmpty(t.GetTableName()))
